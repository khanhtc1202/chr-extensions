...
let SPAM_MESSAGES = ['Nothing', 'wwwww', 'www', 'おおおおお', 'わぁぁぁ'];
...
function getDisplayableMessages() {
  let texts = document.body.innerText.split('\n');
  let newMessages = texts.filter(function (message) {
...
function sendSpamAfter(sender, timeInMs) {
  sendMessageAfter(sender, SPAM_MESSAGES[Math.floor(Math.random() * SPAM_MESSAGES.length)], timeInMs);
}
...
function onNikoStart() {
  ...
  intervalPtr = setInterval(function() {
    ...
    let sendingRangeTime = 50 * messages.length;
